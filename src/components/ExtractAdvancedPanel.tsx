...
export default function ExtractAdvancedPanel({
  ...
  const addFieldToForm = () => {
    ...
    if (!name) return;
    setForm((prev) => ({ ...prev, fields: [...prev.fields, { name, type: newFieldType }] }));
    setNewFieldName("");
  };
  ...
  const removeFormField = (idx: number) => {
    setForm((prev) => {
      ...
      const routingField = prev.routingField && fields.some((f) => f.name === prev.routingField) ? prev.routingField : "";
      return { ...prev, fields, routingField };
    });
  };
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {form.fields.map((f, idx) => (
                <span key={`${f.name}-${idx}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-emerald-300 text-xs font-bold text-emerald-800">
                  {f.name}
                  <span className="text-[10px] text-emerald-400 normal-case">({f.type})</span>
                  <button type="button" onClick={() => removeFormField(idx)} className="text-rose-500 hover:text-rose-700 ml-0.5">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input
                ...
                className="px-3 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 w-40"
              />
              <select
                value={newFieldType}
                onChange={(e) => setNewFieldType(e.target.value)}
                className="px-3 py-2 rounded-xl border border-slate-200 bg-white text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500"
              >
                {FIELD_TYPES.map((t) => (
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-7 max-w-4xl w-full shadow-2xl border border-slate-200 max-h-[92vh] overflow-y-auto space-y-5">
            ...
            <div className="space-y-3">
              <div className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200">
                <div className="text-xs font-bold text-slate-800 mb-2">Add Records to This Workspace</div>
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => openAddRecords("auto")}
                    className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold inline-flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-white border border-emerald-300 hover:bg-emerald-50 text-emerald-800 text-[11px] font-bold inline-flex items-center gap-1.5"
                  >
              ...
              {addPages.length > 0 && (
                <div className="rounded-2xl border border-slate-200 p-3.5 space-y-2.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-xs font-bold text-slate-700">Pages ({addPages.length})</span>
                    <span className="text-[11px] text-slate-400">added for: {addMode === "auto" ? "auto-filing" : `to ${detail.docs.find((d) => d.id === addTarget)?.title || ""}`}</span>
                  </div>
                  <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                    ...
                    <button
                      ...
                      className="rounded-xl border-2 border-dashed border-emerald-300 bg-emerald-50/60 hover:bg-emerald-100 aspect-[3/4] flex flex-col items-center justify-center gap-1 text-center p-2"
                    >
                    ...
                    <button
                      ...
                      className="rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-white aspect-[3/4] flex flex-col items-center justify-center gap-1 text-center p-2"
                    >
                  ...
                  {addMode === "pick" && detail.docs.length > 0 && (
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 flex-1"
                      >
                        {detail.docs.map((d) => (
                  ...
                  <div className="flex justify-end">
                    ...
                    >
                      ...
                      {isExtracting ? "AI Extracting & Filing..." : "Extract & Save"}
                    </button>
                  </div>
                </div>
              )}
